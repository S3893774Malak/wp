...
$("#submit-form").submit(function (e) {
  ...
  if (valid) {
    if ($("input[type='radio'][name='remember']").is(":checked")) {
      var personal_info = [];
      personal_info[0] = $("#user_name").val();
      personal_info[1] = $("#user_email").val();
      personal_info[2] = $("#user_phone").val();
      localStorage.setItem("personal_info", JSON.stringify(personal_info));
    }
    if ($("input[type='radio'][name='forget']").is(":checked")) {
...
$(document).on("change", "#forget", function () {
  if (this.checked) {
